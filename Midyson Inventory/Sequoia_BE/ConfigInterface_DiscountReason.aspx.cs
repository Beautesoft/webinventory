...
{
    public partial class ConfigInterface_DiscountReason : System.Web.UI.Page
    {
        ...
        private void Get_ControlPrefixs()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //GET Method  
                string api = "api/ControlNos?filter={\"where\":{\"controlDescription\":\"Discount Voucher\"}}";
        ...
        protected void Get_BloackReasons()
        {

            //HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format("http://103.253.14.203:3000/api/PaymentRemarks"));
            HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format(System.Configuration.ConfigurationManager.AppSettings["uri"] + "api/PaymentRemarks"));
            WebReq.Method = "GET";
            ...
            List<DiscountReasonUpdate> items = new List<DiscountReasonUpdate>();
            items = JsonConvert.DeserializeObject<List<DiscountReasonUpdate>>(jsonString);
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                Col_1.InnerText = items[i].rCode;
                _Row.Controls.Add(Col_1);
                HtmlTableCell Col_2 = new HtmlTableCell();
                Col_2.Attributes.Add("style", "width: 50 %;");
                Col_2.InnerText = items[i].rDesc;
                _Row.Controls.Add(Col_2);
                HtmlTableCell Col_3 = new HtmlTableCell();
                Col_3.Attributes.Add("style", "width: 10 %; text-align:center");
                if (items[i].isactive)
                {
                    Col_3.InnerHtml = "<input type='checkbox' checked class='chk_DiscountReason editor - active'>";
                }
                else
                {
                    Col_3.InnerHtml = "<input type='checkbox' class='chk_DiscountReason editor - active'>";
                }
                ...
                Col_Action.InnerHtml = "<div class='tools'><i class='edit_DiscountReason glyphicon glyphicon-pencil'></i></div>";
                ...
                Col_4.Attributes.Add("style", "width: 0px; display:none");
                _Row.Controls.Add(Col_4);
                tblDiscountReasons.Rows.Add(_Row);
