...
{
    public partial class ConfigInterface_BlockReason : System.Web.UI.Page
    {
        ...
        private void Get_ControlPrefixs()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //GET Method  
                string api = "api/ControlNos?filter={\"where\":{\"controlDescription\":\"Block Reason\"}}";
        ...
        protected void Get_BloackReasons()
        {
            ...
            //HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format("http://103.253.14.203:3000/api/BlockReasons"));
            HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format(System.Configuration.ConfigurationManager.AppSettings["uri"] + "api/BlockReasons"));
            WebReq.Method = "GET";
            ...
            List<BlockReasonUpdate> items = new List<BlockReasonUpdate>();
            items = JsonConvert.DeserializeObject<List<BlockReasonUpdate>>(jsonString);
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                Col_1.InnerText = items[i].bNo;
                _Row.Controls.Add(Col_1);
                HtmlTableCell Col_2 = new HtmlTableCell();
                Col_2.Attributes.Add("style", "width: 50 %;");
                Col_2.InnerText = items[i].bReason;
                _Row.Controls.Add(Col_2);
                HtmlTableCell Col_3 = new HtmlTableCell();
                Col_3.Attributes.Add("style", "width: 10 %; text-align:center");
                if (items[i].active)
                {
                    Col_3.InnerHtml = "<input type='checkbox' checked class='chk_BlockReason editor - active'>";
                }
                else
                {
                    Col_3.InnerHtml = "<input type='checkbox' class='chk_BlockReason editor - active'>";
                }
                ...
                Col_Action.InnerHtml = "<div class='tools'><i class='edit_BlockReason glyphicon glyphicon-pencil'></i></div>";
