...
{
    public partial class ConfigInterface_Diagnosis : System.Web.UI.Page
    {
        ...
        private void Get_ControlPrefixs()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //GET Method  
                string api = "api/ControlNos?filter={\"where\":{\"controlDescription\":\"Diagnosis\"}}";
        ...
        protected void Get_Diagnosis()
        {

            //HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format("http://103.253.14.203:3000/api/Diagnosissetups"));
            HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format(System.Configuration.ConfigurationManager.AppSettings["uri"] + "api/Diagnosissetups"));
            WebReq.Method = "GET";
            ...
            List<DiagnosisUpdate> items = new List<DiagnosisUpdate>();
            items = JsonConvert.DeserializeObject<List<DiagnosisUpdate>>(jsonString);
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                Col_1.InnerText = items[i].diagCode;
                _Row.Controls.Add(Col_1);
                HtmlTableCell Col_2 = new HtmlTableCell();
                Col_2.Attributes.Add("style", "width: 50 %;");
                Col_2.InnerText = items[i].diagDesc;
                _Row.Controls.Add(Col_2);
                HtmlTableCell Col_3 = new HtmlTableCell();
                Col_3.Attributes.Add("style", "width: 10 %; text-align:center");
                if (items[i].diagIsactive)
                {
                    Col_3.InnerHtml = "<input type='checkbox' checked class='chk_Diagnosis editor - active'>";
                }
                else
                {
                    Col_3.InnerHtml = "<input type='checkbox' class='chk_Diagnosis editor - active'>";
                }
                ...
                Col_Action.InnerHtml = "<div class='tools'><i class='edit_Diagnosis glyphicon glyphicon-pencil'></i></div>";
