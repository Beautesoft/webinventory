...
{
    public partial class ConfigInterface_TransactionReason : System.Web.UI.Page
    {
        ...
        private void Get_ControlPrefixs()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //GET Method  
                string api = "api/ControlNos?filter={\"where\":{\"controlDescription\":\"Transaction Reason\"}}";
        ...
        protected void Get_TransactionReasons()
        {

            //HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format("http://103.253.14.203:3000/api/TransactionReasons"));
            HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format(System.Configuration.ConfigurationManager.AppSettings["uri"] + "api/TransactionReasons"));
            WebReq.Method = "GET";
            ...
            List<TransactionReasonUpdate> items = new List<TransactionReasonUpdate>();
            items = JsonConvert.DeserializeObject<List<TransactionReasonUpdate>>(jsonString);
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                Col_1.InnerText = items[i].reasonCode;
                _Row.Controls.Add(Col_1);
                HtmlTableCell Col_2 = new HtmlTableCell();
                Col_2.Attributes.Add("style", "width: 50 %;");
                Col_2.InnerText = items[i].reasonDesc;
                _Row.Controls.Add(Col_2);
                HtmlTableCell Col_3 = new HtmlTableCell();
                Col_3.Attributes.Add("style", "width: 10 %; text-align:center");
                if (items[i].isactive)
                {
                    Col_3.InnerHtml = "<input type='checkbox' checked class='chk_TransactionReason editor - active'>";
                }
                else
                {
                    Col_3.InnerHtml = "<input type='checkbox' class='chk_TransactionReason editor - active'>";
                }
                ...
                Col_Action.InnerHtml = "<div class='tools'><i class='edit_TransactionReason glyphicon glyphicon-pencil'></i></div>";
                ...
                Col_4.Attributes.Add("style", "width: 0px; display:none");
                _Row.Controls.Add(Col_4);
                tblTransactionReason.Rows.Add(_Row);
