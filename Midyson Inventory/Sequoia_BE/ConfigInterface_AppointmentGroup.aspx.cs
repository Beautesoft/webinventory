...
{
    public partial class ConfigInterface_AppointmentGroup : System.Web.UI.Page
    {
        ...
        private void Get_ControlPrefixs()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //GET Method  
                string api = "api/ControlNos?filter={\"where\":{\"controlDescription\":\"Appointment Group\"}}";
        ...
        protected void Get_AppointmentChannel()
        {

            //HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format("http://103.253.14.203:3000/api/AppointmentGroups"));
            HttpWebRequest WebReq = (HttpWebRequest)WebRequest.Create(string.Format(System.Configuration.ConfigurationManager.AppSettings["uri"] + "api/AppointmentGroups"));
            WebReq.Method = "GET";
            ...
            List<AppointmentGroupUpdate> items = new List<AppointmentGroupUpdate>();
            items = JsonConvert.DeserializeObject<List<AppointmentGroupUpdate>>(jsonString);
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                Col_1.InnerText = items[i].apptGroupCode;
                _Row.Controls.Add(Col_1);
                HtmlTableCell Col_2 = new HtmlTableCell();
                Col_2.Attributes.Add("style", "width: 50 %;");
                Col_2.InnerText = items[i].apptGroupDesc;
                _Row.Controls.Add(Col_2);
                //HtmlTableCell Col_3 = new HtmlTableCell();
                //Col_3.Attributes.Add("style", "width: 50 %;");
                //Col_3.InnerText = items[i].apptGroupSeq;
                //_Row.Controls.Add(Col_3);
                HtmlTableCell Col_3 = new HtmlTableCell();
                Col_3.Attributes.Add("style", "width: 10 %; text-align:center");
                if (items[i].apptGroupIsactive)
                {
                    Col_3.InnerHtml = "<input type='checkbox' checked class='chk_AppointmentGroup editor - active'>";
                }
                else
                {
                    Col_3.InnerHtml = "<input type='checkbox' class='chk_AppointmentGroup editor - active'>";
                }
                _Row.Controls.Add(Col_3);
                HtmlTableCell Col_Action = new HtmlTableCell();
                Col_Action.Attributes.Add("style", "width: 10 %; text-align:center");
                Col_Action.InnerHtml = "<div class='tools'><i class='edit_AppointmentGroup glyphicon glyphicon-pencil'></i></div>";
                ...
                Col_4.Attributes.Add("style", "width: 0px; display:none");
                _Row.Controls.Add(Col_4);
                tblAppointmentGroup.Rows.Add(_Row);
